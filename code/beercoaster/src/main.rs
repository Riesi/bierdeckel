...
fn main() {
    ...
    let static_characteristic = service
        .lock()
        .create_characteristic(MTU_UUID, NimbleProperties::READ);
    let _ = ble_device.set_preferred_mtu(
        esp_idf_svc::sys::BLE_ATT_MTU_MAX
            .try_into()
            .unwrap_or(23u16),
    );
                    ...
                    let event = match ctrl {
                        ...
                        OTAControl::FLASH => {
                            ctrl_animation
                                .lock()
                                .unwrap()
                                .push_back(LedState::BtFlashing);
                            OTAEvent::FlashData
                        }
                        OTAControl::ABORT => OTAEvent::Abort,
                        OTAControl::DONE => {
    ...
    let writable_characteristic = service
        .lock()
        .create_characteristic(WRITE_UUID, NimbleProperties::READ | NimbleProperties::WRITE);
    ...
    bt_wait.add_pattern(LedPattern::new(800, array::repeat(led_animation::BLUE_H)));
    bt_wait.add_pattern(LedPattern::new(200, array::repeat(led_animation::BLACK)));

    let mut active_pattern = LedAnimation::new(2);
    active_pattern.add_pattern(LedPattern::new(1500, array::repeat(led_animation::GREEN_H)));
    active_pattern.add_pattern(LedPattern::new(400, array::repeat(led_animation::BLACK)));

    let mut bt_flashing = LedAnimation::new(4);
    bt_flashing.add_pattern(LedPattern::new(300, array::repeat(led_animation::YELLOW_H)));
    bt_flashing.add_pattern(LedPattern::new(200, array::repeat(led_animation::BLACK)));
    let mut bt_verified = LedAnimation::new(4);
    bt_verified.add_pattern(LedPattern::new(300, array::repeat(led_animation::PINK_H)));
    bt_verified.add_pattern(LedPattern::new(200, array::repeat(led_animation::BLACK)));
    let mut error_pattern = LedAnimation::new(4);
    error_pattern.add_pattern(LedPattern::new(300, array::repeat(led_animation::RED)));
    error_pattern.add_pattern(LedPattern::new(200, array::repeat(led_animation::BLACK)));
    ...
    animation_queue
        .lock()
        .unwrap()
        .push_back(LedState::DefaultPattern);
